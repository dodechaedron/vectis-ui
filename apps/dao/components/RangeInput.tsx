...
export interface RangeInputProps<FV extends FieldValues, FieldName extends Path<FV>> {
  // Required fields
  min: number;
  max: number;
...
export const RangeInput = <FV extends FieldValues, FieldName extends Path<FV>>({
  ...
  const updatePendingValue = useCallback(
    (mouseX: number) => {
      ...
      const unclampedNewValue = Math.round((offsetX / width) * (max - min) + min);
  ...
  }, [disabled, fieldName, isDragging, onChange, pendingNewValue, setValue, updatePendingValue, value]);
  ...
  return (
    <div
      className={clsx(
        "relative h-8 overflow-hidden rounded-sm bg-zinc-400 transition",
        disabled
    ...
    >
      {/* Progress bar. */}
      <div
        className={clsx("absolute top-0 left-0 bottom-0 h-full rounded-sm transition", disabled || noValue ? "bg-zinc-400" : "bg-green-400/50")}
        style={{
          width: `calc(${((pendingNewValue ?? value) - min) / (max - min)} * 100%)`,
        }}
      ></div>
      {/* Value */}
      <p
